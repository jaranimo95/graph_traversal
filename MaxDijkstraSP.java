...
public class MaxDijkstraSP {
     ...
     */
    public MaxDijkstraSP(EdgeWeightedGraph G, int s) {
        for (Edge e : G.edges()) {
            if (e.getBandwidth() < 0)
    ...
    private void relax(Edge e) {
        ...
        if (distTo[w] < distTo[v] + e.getBandwidth()) {
            distTo[w] = distTo[v] + e.getBandwidth();
            edgeTo[w] = e;
    ...
    private boolean check(EdgeWeightedGraph G, int s) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (Edge e : G.adj(v)) {
                int w = e.other(v);
                if (distTo[v] + e.getBandwidth() > distTo[w]) {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        MaxDijkstraSP sp = new MaxDijkstraSP(g, s);
